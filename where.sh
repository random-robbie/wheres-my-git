...
cat $1 | parallel -j 25 --bar curl -skL --connect-timeout 10 | grep -Eo "(http|https)://[\da-z./?A-Z0-9\D=_-]*/" >>/tmp/targets.tmp
wait
# Test for .git/config files and output to found.txt
echo "Testing new targets..."
cat $1 | parallel -j 25 --bar check_target | tee found-jon.txt
rm /tmp/targets.tmp
